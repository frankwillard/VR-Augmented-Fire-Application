...
/**
 * Video controll (modified for this PRICE)
*/
...
AFRAME.registerComponent("immersive-video", {
  ...
  update: function(oldData) {
    ...
    if (self.video_type.stereo) {
      ...
      self.stereo_left_sphere.setAttribute(
        "geometry",
        "primitive:sphere; radius:100; segmentsWidth: 64; segmentsHeight:64"
      );
      self.stereo_left_sphere.setAttribute("material", {
        shader: "flat",
        src: "#" + self.video_id,
        side: "back"
      });
      ...
      self.stereo_left_sphere.setAttribute("rotation", {
        x: 0,
        y: self.el.getAttribute("rotation").y,
        z: 0
      });

      AFRAME.utils.entity.setComponentProperty(
        self.stereo_left_sphere,
        "stereo",
        {
          eye: "left",
          mode: self.video_type.coverage,
          split: self.video_type.split
        }
      );
      ...
      self.stereo_right_sphere.setAttribute(
        "geometry",
        "primitive:sphere; radius:100; segmentsWidth: 64; segmentsHeight:64"
      );
      self.stereo_right_sphere.setAttribute("material", {
        shader: "flat",
        src: "#" + self.video_id,
        side: "back"
      });
      ...
      self.stereo_right_sphere.addEventListener(
      ...
      //      console.log("MONO VIDEO id:",self.video_id);
      ...
      self.mono_sphere.setAttribute(
        "geometry",
        "primitive:sphere; radius:100; segmentsWidth: 64; segmentsHeight:64"
      );
      self.mono_sphere.setAttribute("material", {
        shader: "flat",
        src: "#" + self.video_id,
        side: "back"
      });
...
AFRAME_UIPACK.UIPACK_CONSTANTS = {
  button_hover: 1000,
  menu_pitch: 40, //orig -90
  menu_begin_pitch: 20, //orig 20
  menu_distance: 1.3,
  menu_tick_check: 1,
  offset_player: 0.5,    // MSH moved player up from 0.3
  offset_buttons: 0.9,   // original 0.9
  offset_icons: 0.15,    // original 0.15
  icon_spacing: 0.05,
...
AFRAME.registerComponent("uipack-menu", {
  ...
  init: function() {
    ...
    self.open_icon.setAttribute("uipack-button", {
      icon_name: "bars.png",
      theme: self.data.theme
    });
    ...
    for (var i = 0; i < self.data.icons.length; i++) {
      ...
      my_icon.addEventListener("clicked", function() {
        if (self.open_menu && !self.hidden) {
          self.el.emit(
            "clicked",
            { type: "icon", index: this.icon_index },
            false
          );
        }
      });
    ...
    for (i = 0; i < self.data.buttons.length; i++) {
      ...
      my_button.addEventListener("clicked", function() {
        if (self.open_menu && !self.hidden) {
          self.el.emit(
            "clicked",
            { type: "button", index: this.button_index },
            false
          );
        }
      });
  ...
  update: function(oldData) {
    ...
    var icon_row_half_width =
      ((self.icon_row.length - 1) *
        (AFRAME_UIPACK.UIPACK_CONSTANTS.button_radius * 2) +
        (self.icon_row.length - 1) *
          AFRAME_UIPACK.UIPACK_CONSTANTS.icon_spacing) /
      2.0;

    for (var i = 0; i < self.icon_row.length; i++) {
      self.icon_row[i].setAttribute("uipack-button", {
        icon_name: self.data.icons[i]
      });

      self.icon_row[i].setAttribute(
        "position",
        i *
          (AFRAME_UIPACK.UIPACK_CONSTANTS.button_radius * 2 +
            AFRAME_UIPACK.UIPACK_CONSTANTS.icon_spacing) -
          icon_row_half_width +
          " " +
          AFRAME_UIPACK.UIPACK_CONSTANTS.offset_icons +
          " 0"
      );
    }
    ...
    var button_row_half_width =
      ((self.button_row.length - 1) *
        AFRAME_UIPACK.UIPACK_CONSTANTS.menu_button_width +
        (self.button_row.length - 1) *
          AFRAME_UIPACK.UIPACK_CONSTANTS.button_spacing) /
      2.0;

    for (i = 0; i < self.button_row.length; i++) {
      self.button_row[i].setAttribute("uipack-textbutton", {
        text: self.data.buttons[i],
        width: AFRAME_UIPACK.UIPACK_CONSTANTS.menu_button_width,
        color: "#FFF",
        background: "#000"
      });

      self.button_row[i].setAttribute(
        "position",
        i *
          (AFRAME_UIPACK.UIPACK_CONSTANTS.menu_button_width +
            AFRAME_UIPACK.UIPACK_CONSTANTS.button_spacing) -
          button_row_half_width +
          " " +
          AFRAME_UIPACK.UIPACK_CONSTANTS.offset_buttons +
          " 0"
      );
    }
    ...
    if (self.data.media_id != "") {
      self.media_controls.setAttribute("uipack-mediacontrols", {
        src: "#" + self.data.media_id,
      ...
      self.media_controls.setAttribute(
        "position",
        "0 " + AFRAME_UIPACK.UIPACK_CONSTANTS.offset_player + " 0.0"  
      );
    }
  ...
  tick: function(t) {
    ...
    if (
      self.frame_count % AFRAME_UIPACK.UIPACK_CONSTANTS.menu_tick_check ===
      0
    ) {
      ...
      if (
        camera_pitch < self.data.pitch_max &&
        camera_pitch > self.data.pitch_min
      ) {
        self.hidden = false;
      } else {
        if (!self.open_menu) {
      ...
      if (self.data.open) {
        self.y_position =
          AFRAME_UIPACK.UIPACK_CONSTANTS.menu_distance *
          Math.sin((this.data.pitch * Math.PI) / 180.0);
        self.x_position =
          AFRAME_UIPACK.UIPACK_CONSTANTS.menu_distance *
          Math.cos((this.data.pitch * Math.PI) / 180.0) *
          Math.cos((camera_yaw * Math.PI) / 180.0);
        self.z_position =
          -AFRAME_UIPACK.UIPACK_CONSTANTS.menu_distance *
          Math.cos((this.data.pitch * Math.PI) / 180.0) *
          Math.sin((camera_yaw * Math.PI) / 180.0);

        this.container.setAttribute(
          "position",
          [self.x_position, self.y_position, self.z_position].join(" ")
        );
        ...
        if (self.camera) {
          ...
          self.container.setAttribute("rotation", {
            x: this.data.pitch,
            y: camera_rotation.y,
            z: 0
          });
...
AFRAME.registerComponent("uipack-button", {
  schema: {
    ...
    arc_color: { type: "string", default: "yellow" },
  ...
  init: function() {
    ...
    self.button_mode =
      AFRAME_UIPACK && "cursor_mode" in AFRAME_UIPACK
        ? AFRAME_UIPACK.cursor_mode
        : "desktop";

    // Create the element

    self.button = document.createElement("a-circle");
    self.button.setAttribute("id","vcrcircles"); //this one element name serves both circles in the vcr controls..Sandy
    ...
    if (self.button_mode === "desktop") {
      ...
      this.el.addEventListener("raycaster-intersected", function(event) {
        ...
        if (self.first_hover) {
          ...
          self.ring.setAttribute(
            "material",
            "color:" +
              (self.data.theme
                ? AFRAME_UIPACK.themes[self.data.theme].arc_color
                : self.data.arc_color)
          );
          ...
          if (
            AFRAME.version.startsWith("0.9.") ||
            parseInt(AFRAME.version.split(".")[0]) > 1
          ) {
  ...
  update: function(oldData) {
    ...
    self.icon_path =
      (self.data.theme !== ""
        ? AFRAME_UIPACK.themes[self.data.theme].icon_path
        : AFRAME_UIPACK.UIPACK_CONSTANTS.icon_path) +
      "/" +
      self.data.icon_name;

    self.button.setAttribute("material", {
      src: "url(" + self.icon_path + ")",
      shader: "flat"
    });
    ...
    if (self.el.parentEl === self.el.sceneEl || self.data.absolute_pos) {
      self.x_position =
        self.data.distance * Math.cos((this.data.yaw * Math.PI) / 180.0);
      self.y_position = self.data.elevation;
      self.z_position =
        -self.data.distance * Math.sin((this.data.yaw * Math.PI) / 180.0);
...
AFRAME.registerComponent("uipack-mediacontrols", {
  ...
  init: function() {
    ...
    self.icon.setAttribute("uipack-button", {
      theme: self.data.theme,
      icon_name: AFRAME_UIPACK.UIPACK_CONSTANTS.play_icon,
      radius: self.data.button_radius
    });
    ...
    if (this.video_el.paused) {
      self.icon.setAttribute(
        "uipack-button",
        "icon_name",
        AFRAME_UIPACK.UIPACK_CONSTANTS.play_icon
      );
    } else {
      self.icon.setAttribute(
        "uipack-button",
        "icon_name",
        AFRAME_UIPACK.UIPACK_CONSTANTS.pause_icon
      );
    }
    ...
    window.addEventListener(
      ...
      function(event) {
        switch (event.keyCode) {
          ...
          case 40:
            self.current_step =
              self.current_step > 0 ? self.current_step - 1 : self.current_step;
            self.position_time_from_steps();
            break;
        }
      },
    ...
    this.bar = document.createElement("a-plane"); //set an id so we can explicity hit it .. Sandy
    this.bar.setAttribute("id","vcrbar"); 

    this.bar.setAttribute(
      "color",
      self.data.theme
        ? AFRAME_UIPACK.themes[self.data.theme].player_background
    ...
    if (self.button_mode === "desktop") {
      ...
      this.bar.addEventListener("raycaster-intersected-cleared", function(
        event
      ) {
        self.first_hover = true;
    ...
    this.back_plane.setAttribute("material", {
      color: self.data.theme
        ? AFRAME_UIPACK.themes[self.data.theme].player_background
        : self.data.backgroundColor,
      shader: "flat"
    });
  ...
  update: function(oldData) {
    ...
    self.bar.setAttribute("position", {
      x: self.data.button_radius * 2,
      y: 0,
      z: 0.01
    });
    ...
    self.icon.setAttribute("position", {
      x: -(this.data.width / 2) + self.data.button_radius * 2,
      y: 0,
      z: 0.01
    });
  },
    ...
    if (typeof this.last_time === "undefined" || t - this.last_time > 250) {
      ...
      if (this.video_el.readyState > 0) {
        ...
        if (this.video_el.buffered.length > 0) {
          ...
          ctx.fillStyle = self.data.theme
            ? AFRAME_UIPACK.themes[self.data.theme].player_background
            : self.data.backgroundColor;
          ctx.fillRect(0, 0, this.bar_canvas.width, this.bar_canvas.height);

          // Display time info text
          ctx.font = self.data.theme
            ? AFRAME_UIPACK.themes[self.data.theme].player_font
            : this.data.timeTextFont;
          ctx.fillStyle = self.data.theme
            ? AFRAME_UIPACK.themes[self.data.theme].player_text_color
            : self.data.textColor;
          ctx.textAlign = "center";
          ctx.fillText(
            time_info_text,
            this.bar_canvas.width * 0.45,
            this.bar_canvas.height * 1.0  // MSH dropped down
          );
          ...
          for (var i = 0; i < this.video_el.buffered.length; i++) {
            ...
            ctx.fillStyle = "transparent"; // MSH - made buffering transparent (was grey)
            ctx.fillRect(startX, 0, width, this.bar_canvas.height / 2);
          }

          // MSH add white bar with full range
...
AFRAME_UIPACK.utils = {
  ...
  set_cursor: function(scene, theme) {
    ...
    self.cursor.setAttribute("geometry", {
      primitive: "ring",
      radiusInner: 0.01,
      radiusOuter: 0.02
    });
    self.cursor.setAttribute("material", {
      color: self.theme_data.cursor_color,
      shader: "flat"
    });
    ...
    if (headset && !mobile) {
      ...
      self.laser_controls.setAttribute("line", {
        color: self.theme_data.cursor_color
      });
      self.laser_controls.setAttribute("raycaster", { objects: ".boxGrabbable, .hotspotGrabbable, #Closebox, #vcrbar, #vcrcircles"}); // sandy to limit raycasting..note vcrcircles deal with both play and minimizing circles in the controls
